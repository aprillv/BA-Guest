    ...
    if (self) {
        ...
        switch (action) {
            case PasscodeActionSet:
                ...
                self.title = NSLocalizedString(@"BuildersAccess", nil);
                _enterPrompt = NSLocalizedString(@"Enter PIN", nil);
...
//    if (contentView.frame.size.height>480) {
//        UIImage *baim = [UIImage imageNamed:@"ba.png"];
//        UIImageView *a =[[UIImageView alloc] initWithImage:baim];
//        a.frame=CGRectMake(98, PROMPT_HEIGHT+DIGIT_HEIGHT +MESSAGE_HEIGHT+20, 120, 80);
...
//        UIImage *baim = [UIImage imageNamed:@"ba.png"];
//        UIImageView *a =[[UIImageView alloc] initWithImage:baim];
//        a.frame=CGRectMake(118, PROMPT_HEIGHT+DIGIT_HEIGHT+40 , 72, 48);
//        failedImageView.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin|UIViewAutoresizingFlexibleRightMargin;
//        [contentView addSubview:a];
//    }
    ...
    if (_failedAttempts<4) {
        ...
        if (_failedAttempts <3) {
            if (_failedAttempts==2) {
                failedAttemptsLabel.numberOfLines=3;
                 failedAttemptsLabel.text =@"Incorrect PIN. 1 attempt left. \nIf you fail this time your BuildersAccess information will be reset.";
                ...
                UIImage *failedBg = [[UIImage imageNamed:@"error"] resizableImageWithCapInsets:UIEdgeInsetsMake(0, FAILED_LCAP, 0, FAILED_RCAP)];
                [failedImageView setImage:failedBg];
                failedImageView.frame = CGRectMake(x, y-10, bgWidth, FAILED_HEIGHT+34);
//                x = failedImageView.frame.origin.x+FAILED_MARGIN;
                ...
                failedAttemptsLabel.frame = CGRectMake(10, y, 310, 60);
            }else{
             failedAttemptsLabel.text =[NSString stringWithFormat:NSLocalizedString(@"Incorrect PIN. %i attempts left", nil),(3- _failedAttempts)];
                [failedAttemptsLabel sizeToFit];
                CGFloat bgWidth = failedAttemptsLabel.bounds.size.width + FAILED_MARGIN*2;
                CGFloat x = floor((contentView.bounds.size.width-bgWidth)/2);
                CGFloat y = PROMPT_HEIGHT+DIGIT_HEIGHT+floor((MESSAGE_HEIGHT-FAILED_HEIGHT)/2);
...
//                x = failedImageView.frame.origin.x+FAILED_MARGIN;
                y = failedImageView.frame.origin.y+floor((failedImageView.bounds.size.height-failedAttemptsLabel.frame.size.height)/2);
                failedAttemptsLabel.frame = CGRectMake(10, y, 310, failedAttemptsLabel.bounds.size.height);
            }
            ...
            failedAttemptsLabel.text = @"No remaining attempts";
             [_delegate g:self];
        }
