...
//        self.indicator = MB_AUTORELEASE([[MBBarProgressView alloc] init]);
        self.indicator=[[UIProgressView alloc]initWithFrame:CGRectMake(0, 10, 150, 30)];
...
//self.detailsLabelText=[NSString stringWithFormat:@"%f%%",progress];
//}
#pragma mark - Layout
...
//	}
    ...
    if (self.mode==MBProgressHUDModeDeterminateHorizontalBar) {
//        indicatorF.origin.x = roundf((bounds.size.width - indicatorF.size.width-60) / 2) + xPos;
        indicatorF.origin.x = roundf((bounds.size.width - labelSize.width) / 2) + xPos;
        indicatorF.origin.y = yPos-5;
        indicatorF.size.width=labelSize.width-40;
    }else{
    indicatorF.origin.x = roundf((bounds.size.width - indicatorF.size.width) / 2) + xPos;
        indicatorF.origin.y = yPos;
//        NSLog(@"%f", indicatorF.origin.x);
    }
...
//	detailsLabelF.origin.x = roundf((bounds.size.width - detailsLabelSize.width) / 2) + xPos;
    detailsLabelF.origin.y=indicator.frame.origin.y-5;
    detailsLabelF.origin.x=indicatorF.origin.x+10+indicatorF.size.width;
    
	detailsLabelF.size = detailsLabelSize;
    detailsLabelF.size.width=40;
...
- (void)updateUIForKeypath:(NSString *)keyPath {
	if ([keyPath isEqualToString:@"mode"] || [keyPath isEqualToString:@"customView"]) {
		...
		if ([indicator respondsToSelector:@selector(setProgress:)]) {
            detailsLabel.text = [NSString stringWithFormat:@"%d%%", [[NSNumber numberWithFloat:progress*100] intValue]];
